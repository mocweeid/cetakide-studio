          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 sm:p-5 backdrop-blur-md">
             ...
             {tasks.length === 0 ? (
                ...
                <div className="space-y-3">
                   {tasks.map((t, idx) => (
                      <div key={t.id} className="flex items-center justify-between bg-black/40 border border-white/10 p-3 rounded-xl">
                         <div>
                         ...
                         {!generating && !done && (
                            <button onClick={() => handleRemoveTask(t.id)} className="text-red-400 hover:text-red-300 p-2 rounded-md hover:bg-white/5">
                   ...
                   {!generating && !done && (
                      <div className="flex justify-end pt-2">
                         <div className="text-right">
                           ...
                           <p className="text-lg font-bold text-primary">Rp {totalCost.toLocaleString("id-ID")}</p>
                         </div>
                      </div>
                   )}
                </div>
          ...
          {(generating || done) && (
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 backdrop-blur-md">
              <div className="mb-2 flex items-center justify-between text-sm">
                <span className="font-medium">{done ? "Semua Task Selesai!" : "Memproses Antrean…"}</span>
          ...
          {results.length > 0 && (
            <div className="space-y-4">
               {results.map((r, idx) => {
                  ...
                  return (
                     <div key={r.taskId} className="rounded-2xl border border-white/10 bg-white/[0.04] p-4 sm:p-5 backdrop-blur-md">
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                          {r.urls.map((src, i) => (
                            ...
                            >
                              <img src={src} alt={`Visual ${i + 1}`} className="w-full h-full object-cover" />
