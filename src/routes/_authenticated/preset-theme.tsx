...
export const getThemeStyles = (theme: string) => {
  ...
  if (t.includes("glass")) {
    wrapper = "bg-white/5 border border-white/20 backdrop-blur-md rounded-2xl shadow-[0_4px_30px_rgba(0,0,0,0.1)]";
    button = "bg-white/20 text-white border border-white/30 rounded-full";
  } else if (t.includes("neu") || t.includes("soft")) {
    wrapper = "bg-[#e0e5ec] border-transparent rounded-2xl shadow-[8px_8px_16px_#a3b1c6,-8px_-8px_16px_#ffffff] text-gray-800";
    title = "text-gray-800 font-medium";
    button = "bg-[#e0e5ec] text-gray-800 shadow-[4px_4px_8px_#a3b1c6,-4px_-4px_8px_#ffffff] rounded-lg";
  } else if (t.includes("clay")) {
    wrapper = "bg-[#f3f4f6] border-transparent rounded-[2rem] shadow-[inset_8px_8px_16px_#ffffff,inset_-8px_-8px_16px_#d1d5db,8px_8px_16px_#d1d5db] text-gray-800";
    title = "text-gray-800 font-bold";
    button = "bg-[#f3f4f6] text-gray-800 shadow-[inset_4px_4px_8px_#ffffff,inset_-4px_-4px_8px_#d1d5db] rounded-2xl";
  } else if (t.includes("brutal")) {
    wrapper = "bg-yellow-400 border-4 border-black rounded-none shadow-[8px_8px_0px_0px_#000000] text-black";
    title = "text-black font-black uppercase";
    image = "rounded-none border-b-4 border-black";
    button = "bg-black text-white font-bold uppercase rounded-none border-2 border-black hover:bg-gray-800";
    ...
    wrapper = "bg-black border-pink-500 border rounded-none shadow-[0_0_15px_#ec4899] text-pink-500";
    ...
    wrapper = "bg-pink-300 border-4 border-black rounded-xl shadow-[6px_6px_0px_0px_#000000] text-black";
    title = "text-black font-black";
    button = "bg-cyan-400 text-black border-2 border-black rounded-full font-bold hover:bg-cyan-500";
    ...
    button = "bg-[#6200EE] text-white rounded-full uppercase text-xs font-bold shadow-sm hover:bg-[#3700B3]";
  } else if (t.includes("fluent") || t.includes("windows")) {
    wrapper = "bg-white/60 border-white/40 border backdrop-blur-xl rounded-lg shadow-sm text-gray-900";
    title = "text-gray-900";
...
function PresetTheme() {
  ...
  return (
    <AppShell title="Preset Theme" subtitle="Pilih preset visual yang ingin Anda terapkan ke desain" user={user}>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col p-4 flex-1">
                <h2 className={`text-base font-bold flex-1 ${styles.title}`}>
                  {theme}
                </h2>
                <div className="mt-4 flex items-center justify-between">
                  <span className={`px-3 py-1.5 text-xs font-semibold inline-block ${styles.button}`}>
                    Gunakan Preset
                  </span>
                </div>
              </div>
